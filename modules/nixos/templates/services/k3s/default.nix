...
in
{
  ...
  config = let
    ...
    k3sExtraFlags = [ 
      ...
      "--server https://${cfg.head.ip-address}:6443"
    ];
  ...
  in
    ...
    system.activationScripts.writeContainerdConfigTemplate = lib.mkIf cfg.nvidia (stringAfter ["var"] ''
      ...
      services = {
        ...
        minio-init = lib.mkIf cfg.addons.minio.enable {
          ...
          script = ''
            ...
            mc --config-dir "$RUNTIME_DIRECTORY" alias set minio http://${cfg.addons.minio.ip-address}:9000 "$MINIO_ROOT_USER" "$MINIO_ROOT_PASSWORD"
